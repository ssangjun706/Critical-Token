...
class RolloutsCollector:
    def __init__(
        ...
        self.checkpoint_file = os.path.join(output_dir, f"rollouts_checkpoint.json")
        self.output_file = os.path.join(output_dir, f"rollouts_results.json")
        self.resume = resume

        # For rollouts, we don't need high max_logprobs
        self.llm = LLM(
            model=model_name,
            gpu_memory_utilization=0.95,
            dtype="bfloat16",
            max_model_len=4096,
            max_logprobs=1,  # Much smaller for rollouts
        )
    ...
    def collect(self) -> List[Dict]:
        ...
        logger.info(
            f"Starting rollouts collection for {len(self.data)} traces (starting from {start_index})"
        )
        ...
        for idx in range(start_index, len(self.data)):
            ...
            try:
                ...
                if self.use_wandb:
                    wandb.log(
                        {
                            "rollouts_progress": len(results) / len(self.data),
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            f"Rollouts collection completed: Total {len(traces)} traces processed."
        )
    except (Exception, KeyboardInterrupt) as e:
        logger.error(f"Error in rollouts collection: {e}")
    finally:
        collector.shutdown()
